...
async function getLatestOpenQuote(chatId, correlationId) {
  ...
  const quotes = await firestore.queryOpenQuotesByChatId(chatId);
  if (!quotes.length) {
...
async function quoteExistsForVin(chatId, vin, correlationId) {
  const quotes = await firestore.queryOpenQuotesByChatIdAndVin(chatId, vin);
...
async function createQuote(data, correlationId) {
  ...
  return firestore.createQuote(data);
}
...
async function addToBasket(quoteId, data, correlationId) {
  ...
  const existing = await firestore.getBasketItems(quoteId);
  ...
  return firestore.addToBasket(quoteId, data);
}
...
async function getBasketItems(quoteId, correlationId) {
  return firestore.getBasketItems(quoteId);
}
