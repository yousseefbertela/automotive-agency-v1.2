...
async function getState(chatId, correlationId) {
  ...
  const session = await firestore.getSession(chatId);
  ...
  const user = await firestore.queryUserByChatId(chatId);
  ...
  await firestore.upsertSession(chatId, initialState);
...
async function saveState(chatId, data, correlationId) {
  ...
  await firestore.upsertSession(chatId, data);
}
