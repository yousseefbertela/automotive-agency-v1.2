...
const odoo = require('../integrations/odoo.client');
const sheets = require('../integrations/sheets.client');
const firestore = require('../integrations/firestore.client');
 ...
 * 7. Reply to Telegram
 */
async function handlePart(chatId, item, state, correlationId) {
  const log = logger.child(correlationId);
  const partNames = item.part_name || [];

  if (!partNames.length) {
    await telegram.sendMessage(chatId, 'من فضلك حدد قطعة الغيار المطلوبة.');
    return;
  }
  ...
  if (!quote) {
    await telegram.sendMessage(chatId, 'مفيش عرض سعر مفتوح. ابعت الـ VIN الأول.');
    return;
  }

  const vin = quote.vin || state.vin;
  if (!vin) {
    await telegram.sendMessage(chatId, 'مفيش VIN محفوظ. ابعت الـ VIN الأول.');
    return;
  }
  ...
  for (const partName of partNames) {
    try {
      await processOnePart(chatId, partName, vin, quote, state, correlationId);
    } catch (err) {
      log.error('part.flow: error processing part', { partName, error: err.message });
      await telegram.sendMessage(chatId, `حصل مشكلة في البحث عن "${partName}". حاول تاني.`);
    }
  }
  ...
  await telegram.sendMessage(chatId, addMoreMsg);
}

async function processOnePart(chatId, partName, vin, quote, state, correlationId) {
  const log = logger.child(correlationId);
  ...
  if (!chosenPart) {
    ...
    if (!allSubgroups.length) {
      ...
      await telegram.sendMessage(chatId, `مش لاقي نتايج للقطعة "${partName}". جرب اسم تاني.`);
      return;
    }
  ...
  if (!chosenPart || !chosenPart.part_number) {
    await telegram.sendMessage(chatId, `تمام، آسف جداً. مش لاقي القطعة "${partName}". جرب اسم تاني.`);
    return;
  }
  ...
  if (diagramUrl) {
    try {
      ...
      if (imageData) {
        await telegram.sendPhotoBuffer(chatId, imageData.data, `Diagram for ${chosenPart.description || partName}`);
      }
    } catch (err) {
  ...
  if (!products.length) {
    await telegram.sendMessage(chatId, 'Sorry item out of stock or unavailable');
    return;
  }
