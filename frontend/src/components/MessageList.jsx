      ...
      {messages.length === 0 && !loading && (
        <div className="luxury-empty flex flex-col items-center justify-center min-h-[280px] text-center px-4">
          <div className="bmw-logo-center mb-6" aria-hidden>
            <img src="/bmw-hero.png" alt="" className="w-40 h-40 md:w-52 md:h-52 object-contain mx-auto drop-shadow-2xl" />
          </div>
          <p className="text-slate-500 text-sm font-medium theme-light:text-slate-600">Start by typing…</p>
          <p className="text-slate-600 text-xs mt-1 theme-light:text-slate-500">or upload a photo for OCR</p>
        </div>
      )}
      {messages.map((m) => (
